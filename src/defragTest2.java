...
public class defragTest2 {
	...
	public void deFragTest() {
	    ...
	    //Create File_3 write MAX_BUF characters
	  
	    for ( int i=0; i< MAX_BUF; i++ ) buf[i] = c(i);
	  
	    fd = S.open( "File_3", C.O_WRONLY|C.O_CREAT, 0777 );
	    assertTrue("open(\"File_3\", O_WRONLY|O_CREAT, 0777) -> " + fd, fd >= 0);
	    res = S.write( fd, buf, MAX_BUF );
	    assertTrue("write( fd, buf, MAX_BUF ) --> " + res, res == MAX_BUF);
	    res = S.close( fd );
	    assertTrue("S.close( fd ) --> " + res, res == C.OK);
	    ...
	    //Create File_4 write characters till fill disk
	  
	    for ( int i=0; i< MAX_BUF; i++ ) buf[i] = c(i);
	  
	    fd = S.open( "File_4", C.O_CREAT|C.O_WRONLY, 0 );
	    assertTrue("S.open( \"File_4\", O_CREAT|O_WRONLY, 0) -> " + fd, fd >= 0);
	    for (;;)
	    {
	      res = S.write( fd, buf, MAX_BUF );
